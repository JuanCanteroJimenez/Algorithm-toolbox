...
class graph:
    ...
    def BelmanFord2(self,partida, destino):
            ...
            while True:
                busca = referencias[busca].previo
                camino.append(busca-1)
                if busca == partida:
                    break
            print(referencias[destino].distancia*-1)
            camino_bonito = ""
            camino.reverse()
            for x in range(0,len(camino)):
                if x == len(camino)-1:
                    camino_bonito = camino_bonito + str(camino[x])
                else:
                    camino_bonito = camino_bonito + str(camino[x]) + "->"
            print(camino_bonito)
...
def main():
    ...
    while True:
        ...
        if text == "":
            break
        up = re.compile(r"^\d{1,3}")
        vp = re.compile(r"->\d{1,3}")
        ep = re.compile(r"\d{1,3}$")
        u = int(up.search(text).group(0)) + 1
        v = int(vp.search(text).group(0)[2:len(vp.search(text).group(0))]) + 1
        e = int(ep.search(text).group(0)) * -1
        max_edge.append(u)
        max_edge.append(v)
        edges.append([u,v,e])
    grafo = graph(max(max_edge))
    for x in edges:
        grafo.addedge_direct_weigthed(x[0],x[1],x[2])
    grafo.BelmanFord2(partida, destino)
